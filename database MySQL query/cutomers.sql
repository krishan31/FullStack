...
-- select * from products where quantity_in_stock in (49,38,72);   //return in products with QIS equal to 49,38,72

-- between
-- select * from customers where points between 1000 and 3000; 
-- select * from customers where birth_date between "1990/01/01" AND "2000/01/01"; // slect customer born between 1990

-- LIKE
-- select * from customers where address ="TRAIL" or "AVENUE"AND phone Like "%9"; 
-- select * from customers where last_name regexp "e[b-l]"; ^, $, |, [], [-]
...
-- select first_name, last_name, (10*10)+points as point from customers order by points, first_name; 
-- select * from order_items where order_id = 2 and unit_price*quantity  order by order_id desc; 
...
-- join 
-- select * from orders inner join customers on customers.customer_id = orders.customer_id; //select orders from order_id with customer name
...
-- select order_id, p.product_id, o.quantity, o.unit_price, name from order_items o join products p on o.product_id = p.product_id; 
...
-- select p.date, p.amount, c.name, pm.name,  p.invoice_id from sql_invoicing.payments p join sql_invoicing.payment_methods pm on p.payment_method = pm.payment_method_id join sql_invoicing.clients c on p.client_id = c.client_id; //select name amount and payment of three table
...
-- select c.customer_id, c.first_name, o.order_id from customers c left join orders o on c.customer_id=o.customer_id order by c.customer_id desc;
...
-- select order_id, order_date, "Active" as status from orders where order_date>="2019-01-01" UNION select order_id, order_date, "Archives" as status from orders where order_date < "2019-01-01";
